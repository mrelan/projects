...
df5 <- df4 %>% filter(keyword == "chickens for sale")
...
map1<-tm_shape(metro)+tm_borders("gray20", alpha=0.2)+
  ...
  tm_layout(title = "chickens for sale",legend.text.size=0.7, legend.title.size=1)

df6 <- df4 %>% filter(keyword == "chicks for sale")
...
map2<-tm_shape(metro)+tm_borders("gray20", alpha=0.2)+
  ...
  tm_text("location", size=0.7, col="black")+
  tm_layout(title = "chicks for sale", legend.text.size=0.7, legend.title.size=1)

df7 <- df4 %>% filter(keyword == "ducklings for sale")
...
map3<-tm_shape(metro)+tm_borders("gray20", alpha=0.2)+
  tm_shape(trendmap)+tm_fill("hits",palette=colorpal, title=title, breaks=c(0,20,40,60,80,100))+
  tm_text("location", size=0.7, col="black")+
  tm_layout(title = "ducklings for sale",legend.text.size=0.7, legend.title.size=1)

map<-tmap_arrange(map1, map2, map3, nrow=3)
tmap_save(map,
          filename="G:/Shared drives/2020 FIRE-APHIS/INTERMEDIATE/trends_forsale.png",
          dpi=500, )
##################################################################################
words<-c("\"baby poultry for sale\"", 
         "\"chicks for sale\"", 
...
write.csv(allwords, "trends_forsale.csv")  
...
allplots<-ggarrange(plot_analysis[[1]],plot_analysis[[2]], 
                    plot_analysis[[3]],plot_analysis[[5]], 
                    nrow=2, ncol=2, common.legend = TRUE, legend="bottom")
ggsave("G:/Shared drives/2020 FIRE-APHIS/INTERMEDIATE/trendsplot_forsale.png", 
       allplots, dpi=500, 
