...
df5 <- df4 %>% filter(keyword == "fresh eggs")
...
map1<-tm_shape(metro)+tm_borders("gray20", alpha=0.2)+
  ...
  tm_layout(title = "fresh eggs",legend.text.size=0.7, legend.title.size=1)

df6 <- df4 %>% filter(keyword == "hatching eggs")
...
map2<-tm_shape(metro)+tm_borders("gray20", alpha=0.2)+
  ...
  tm_text("location", size=0.7, col="black")+
  tm_layout(title = "hatching eggs", legend.text.size=0.7, legend.title.size=1)

df7 <- df4 %>% filter(keyword == "incubator")
trenddf <- geo_join(metro, df7, 'GEOID', 'GEOID')
trendmap<-trenddf[!is.na(trenddf@data$hits),] #remove empty values

map3<-tm_shape(metro)+tm_borders("gray20", alpha=0.2)+
  tm_shape(trendmap)+tm_fill("hits",palette=colorpal, title=title, breaks=c(0,20,40,60,80,100))+
  tm_text("location", size=0.7, col="black", remove.overlap = TRUE)+
  tm_layout(title = "incubator",legend.text.size=0.7, legend.title.size=1)

map<-tmap_arrange(map1, map2, map3, nrow=3)
tmap_save(map,
          filename="G:/Shared drives/2020 FIRE-APHIS/INTERMEDIATE/trends_eggs.png",
          dpi=500, )
...
write.csv(allwords, "trends_eggs.csv")  
...
plot2<-plot_analysis[[2]]
plot3<-plot_analysis[[3]]
plot4<-plot_analysis[[6]]

allplots<-ggarrange(plot1, plot3, nrow=1, ncol=2, common.legend = TRUE, legend="bottom")
ggsave(filename="G:/Shared drives/2020 FIRE-APHIS/INTERMEDIATE/trendsplot_eggs.png", 
       plot=allplots,
       dpi=500, height = 3, width=5, units="in")
