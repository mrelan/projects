...
#define up to 5 keywords in the same group
words<-c("\"raising chickens\"", 
         "\"backyard chickens\"", 
         "\"raising ducklings\"", 
         "\"backyard flock\"", 
         "\"small flock\"")
...
info <- ogrInfo("G:/Shared drives/2020 FIRE-APHIS/OUTPUT/Shapefiles/metro.shp", require_geomType = "wkbPolygon")
metro<-readOGR("G:/Shared drives/2020 FIRE-APHIS/OUTPUT/Shapefiles/metro.shp",info[["layer"]],
               require_geomType = "wkbPolygon")
col <- rev(brewer.pal(n=7, "RdYlBu"))
colorpal <- col[3:7]

df5 <- df4 %>% filter(keyword == "raising chickens")
...
map1<-tm_shape(metro)+tm_borders("gray20", alpha=0.2)+
  ...
  tm_layout(title = "raising chickens",legend.text.size=0.7, legend.title.size=1)

df6 <- df4 %>% filter(keyword == "backyard chickens")
...
map2<-tm_shape(metro)+tm_borders("gray20", alpha=0.2)+
  ...
  tm_text("location", size=0.7, col="black")+
  tm_layout(title = "backyard chickens", legend.text.size=0.7, legend.title.size=1)

map<-tmap_arrange(map1, map2, nrow=2)
tmap_save(map,
          filename="G:/Shared drives/2020 FIRE-APHIS/INTERMEDIATE/trends_backyard.png",
          dpi=500, )
##################################################################################
words<-c("\"raising chickens\"", 
         "\"backyard chickens\"", 
...
write.csv(allwords, "trends_backyard.csv")  
...
allplots<-ggarrange(plot_analysis[[1]],plot_analysis[[2]], nrow=1, common.legend = TRUE, legend="bottom")
ggsave("G:/Shared drives/2020 FIRE-APHIS/INTERMEDIATE/trendsplot_backyard.png", 
       allplots, dpi=500,
