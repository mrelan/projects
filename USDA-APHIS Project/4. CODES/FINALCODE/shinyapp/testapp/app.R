...
server <- function(input, output) {
    ...
    distance2 <- function(default_lat, default_lon, range){
        dist <- c()
        for (i in 1:nrow(placesdf)) {
            lat <- placesdf[i,]$lat
            lon <- placesdf[i,]$lon
            temp <-
                distm(c(default_lat, default_lon), c(lat, lon), fun = distHaversine)
        ...
        dist <- dist / 1000
        places2 <- cbind(placesdf[2:10], dist)
        list <- places2 %>%
            filter(dist < range)
        return(list)
    }
    output$data_dist2 <- DT::renderDataTable(datatable(
        distance2(as.numeric(input$lat2),as.numeric(input$lon2),as.numeric(input$range2)),
